...
def make_plot():
    ...
    [r_e_p,p_map]=xr_stats_corr(xr_detrend(dp_ycn.sel(time=slice('2001','2020'))),
                   xr_detrend(de_ycn_gleam.sel(time=slice('2001','2020'))), dim='time')
    ...
    im = plot_map(r_e_p.where(cn_mask>0),ax=ax1, levels=np.arange(-1,1.01,0.1), cmap='RdBu_r',
                  lw=2,extent=[73, 137, 15, 53])
    ax1.text(0.5,0.9,'corr(P,E)',transform=ax1.transAxes, fontsize=12,ha='center')
    ...
    plt.savefig('../figure/fig_prec_e_corr_map.png',dpi=300,bbox_inches='tight')
    print('Fig saved')
