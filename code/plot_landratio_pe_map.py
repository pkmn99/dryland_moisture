...
from plot_pe_prec_corr_map import load_e2p_data
...
def make_plot(et_data='GLEAM'):
    ...
    (dep_ymonmeancn_gleam.where(ai_con).weighted(w).mean(dim=['lat','lon'])).plot(ax=ax4)
    (dep_ymonmeancn_gleam_cndry.where(ai_con).weighted(w).mean(dim=['lat','lon'])).plot(ax=ax4)
    (dp_ymonmeancn.where(ai_con).weighted(w).mean(dim=['lat','lon'])).plot(ax=ax4)
    ...
    plt.savefig('../figure/fig_landratio_pe_map_%s.png'%et_data,dpi=300,bbox_inches='tight')
    print('Fig saved')
